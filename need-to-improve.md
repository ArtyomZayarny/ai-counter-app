...
- [ ] `cd ai-counter && python -m pytest tests/` — all pass
- [ ] `cd ai-counter-app && flutter analyze` — no errors
- [ ] Register with password < 8 chars → 422
- [ ] Create tariff with price_per_unit=-1 → 422
- [ ] GET /readings?limit=999999 → capped at 500
- [ ] /recognize error response has no stack trace
- [ ] Create bill with same from/to reading → 400
- [ ] Create bill with zero consumption → 400
- [ ] Manual input > 9999999 → error shown
